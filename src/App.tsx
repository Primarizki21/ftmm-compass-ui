import { Bell, Compass, LayoutDashboard, LogOut, Map, Menu, MessageSquare, Search, User, X } from 'lucide-react';
    ...
    <div className="flex h-screen w-full bg-background overflow-hidden font-sans text-foreground">
      ...
      <aside 
        className={cn(
          ...
          isSidebarOpen ? "w-64 translate-x-0" : "w-64 -translate-x-full md:translate-x-0 md:w-0 overflow-hidden opacity-0"
        )}
      >
        <div className="flex flex-col h-full min-w-[16rem]">
          ...
          <div className="p-6 pb-2">
            <div className="flex justify-center">
              <img src={mainLogo} alt="Main Logo" className="h-20 object-contain drop-shadow-md" />
            </div>
          </div>
          ...
          <div className="px-6 pb-6 pt-4 flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif font-bold text-lg tracking-wide text-gold whitespace-nowrap">FTMM COMPASS</h1>
              <p className="text-[10px] text-teal-light uppercase tracking-wider font-mono whitespace-nowrap">Academic Advisor</p>
            </div>
          </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isActive 
                      ...
                      : "text-slate-400 hover:text-white hover:bg-navy-light/50 border-l-4 border-transparent"
                  )}
                >
                  <Icon className={cn("w-5 h-5 flex-shrink-0", isActive ? "text-gold" : "text-teal-light group-hover:text-teal-light")} />
          ...
          <div className="p-4 border-t border-navy-light/50">
            <div className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-navy-light/50 transition-colors cursor-pointer text-slate-300 hover:text-white">
              ...
              <div className="flex-1 overflow-hidden">
                <p className="text-sm font-medium truncate">Airlangga Student</p>
                <p className="text-xs text-slate-400 font-mono truncate">1621123456</p>
              </div>
              <LogOut className="w-4 h-4 text-slate-500 hover:text-danger flex-shrink-0" />
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="h-16 bg-surface border-b border-border flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-serif text-xl md:text-2xl font-bold text-navy capitalize truncate">
              {activePage.replace('-', ' ')}
            </h2>
          </div>
