...
function normalizeType(raw?: string): 'Wajib' | 'Pilihan' {
  ...
  if (j.includes('pilihan') || j.includes('pengayaan') || j.includes('peminatan')) return 'Pilihan';
  return 'Wajib';
}
...
export const PROGRAMS: string[] = Object.keys(coursesByProgram).sort((a, b) => a.localeCompare(b));
...
export const ALL_COURSES: Course[] = PROGRAMS.flatMap(p => coursesByProgram[p] ?? []);
