...
export default function Chatbot() {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Halo! Saya Compass AI, asisten akademikmu di FTMM Universitas Airlangga. Ada yang bisa saya bantu terkait rencana studimu hari ini?",
    },
  ])
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      let aiResponse =
        "Maaf, saya belum memahami pertanyaan itu. Bisa tolong ulangi?"
      if (
        userMsg.toLowerCase().includes("data science") ||
        userMsg.toLowerCase().includes("rekomendasi")
      ) {
        aiResponse =
          "Untuk track Data Science, saya sangat merekomendasikan mata kuliah: \n1. Machine Learning (II4042)\n2. Deep Learning (II4050)\n3. Data Visualization (II4045)\nPastikan kamu sudah mengambil Aljabar Linear sebagai prasyaratnya ya!"
      } else if (
        userMsg.toLowerCase().includes("krs") ||
        userMsg.toLowerCase().includes("planning")
      ) {
        aiResponse =
          "Tentu, mari kita lihat rencana KRS-mu. Saat ini ada konflik jadwal antara Machine Learning dan Data Visualization di hari Selasa. Ingin saya carikan kelas alternatif?"
      }
    ...
    <div className="h-full flex flex-col bg-surface border border-border rounded-xl shadow-sm overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="p-4 border-b border-border bg-navy text-white flex justify-between items-center relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div>
            <h3 className="font-heading font-bold text-lg">Compass AI</h3>
            <p className="text-xs text-teal-light font-mono">
              Online • Powered by FTMM
            </p>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div
              className={cn(
                ...
                msg.role === "user"
                  ? "bg-teal text-navy-dark rounded-tr-none"
      ...
      <div className="p-4 bg-surface border-t border-border">
        ...
        <p className="text-[10px] text-center text-muted mt-2">
          Compass AI dapat membuat kesalahan. Harap verifikasi informasi penting
          dengan Dosen Wali.
        </p>
      </div>
    </div>
