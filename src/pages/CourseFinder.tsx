...
export default function CourseFinder({ onAddToPlanner, addedCourseIds }: CourseFinderProps) {
      ...
      <div className="bg-surface rounded-xl border border-border shadow-sm p-4 mb-4 space-y-3 flex-shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-border focus:border-gold focus:ring-1 focus:ring-gold outline-none transition-all text-sm font-sans"
            value={searchQuery}
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  <span className={cn(
                    ...
                    course.type === 'Wajib' ? 'bg-navy/8 text-navy' : 'bg-gold/10 text-gold-hover'
                  )}>
                    {course.type}
                  </span>
                </div>
                <p className="text-xs text-muted line-clamp-2 leading-relaxed">{course.desc}</p>
                ...
                {addedCourseIds.has(course.id) && (
                  <div className="flex items-center gap-1.5 text-xs font-medium text-teal border-t border-border pt-2 -mx-0 -mb-0">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Sudah ditambahkan ke planner
                  </div>
                )}
              </div>
            ))}
        ...
        >
          <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh]">
            ...
            <div className="bg-navy text-white p-6 relative overflow-hidden flex-shrink-0">
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <span className="px-2.5 py-1 bg-white/10 rounded-md text-xs font-medium backdrop-blur-sm">
                    {selectedCourse.parity === 'odd' ? 'Sem Ganjil' : 'Sem Genap'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              <div>
                <h3 className="text-xs font-bold text-navy uppercase tracking-wider mb-2">Deskripsi</h3>
                <p className="text-sm text-foreground leading-relaxed">{selectedCourse.desc}</p>
              </div>

              {selectedCourse.prerequisites && selectedCourse.prerequisites.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-navy uppercase tracking-wider mb-2">Prasyarat</h3>
              ...
              {selectedCourse.workload && (
                <div>
                  <h3 className="text-xs font-bold text-navy uppercase tracking-wider mb-2">Beban Kuliah</h3>
                  <div className="bg-background rounded-lg p-3 grid grid-cols-3 gap-3 text-center divide-x divide-border">
                    <div>
                      <div className="text-xl font-serif font-bold text-navy">{selectedCourse.workload.hours}</div>
                      <div className="text-[10px] text-muted mt-0.5">Jam/Sem</div>
                    </div>
                    <div>
                      <div className="text-xl font-serif font-bold text-navy">{selectedCourse.workload.lectureCredits}</div>
                      <div className="text-[10px] text-muted mt-0.5">SKS Kuliah</div>
                    </div>
                    <div>
                      <div className="text-xl font-serif font-bold text-navy">{selectedCourse.workload.labCredits}</div>
                      <div className="text-[10px] text-muted mt-0.5">SKS Praktikum</div>
                    </div>
                  </div>
                </div>
              )}
...
function CourseDetailView({ course, onBack, onAdd, addedCourseIds }: CourseDetailViewProps) {
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-300">
      ...
      <div className="bg-navy text-white rounded-xl p-6 mb-5 relative overflow-hidden flex-shrink-0">
        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <span className="px-3 py-1 bg-white/10 rounded-md text-sm font-medium backdrop-blur-sm">Semester {course.semester}</span>
            ...
            <span className="px-3 py-1 bg-white/10 rounded-md text-sm font-medium backdrop-blur-sm">
              Ditawarkan Sem {course.parity === 'odd' ? 'Ganjil (1, 3, 5, 7)' : 'Genap (2, 4, 6, 8)'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pb-4">
        ...
        <div className="bg-surface rounded-xl border border-border p-6">
          <h3 className="font-serif font-bold text-lg text-navy mb-3">Deskripsi Mata Kuliah</h3>
          <p className="text-sm text-foreground leading-relaxed">{course.desc}</p>
          {course.workload && (
            <div className="mt-5 bg-background rounded-lg p-4 grid grid-cols-3 gap-4 text-center divide-x divide-border">
              <div>
                <div className="text-2xl font-serif font-bold text-navy">{course.workload.hours}</div>
                <div className="text-xs text-muted mt-1">Jam/Semester</div>
              </div>
              <div>
                <div className="text-2xl font-serif font-bold text-navy">{course.workload.lectureCredits}</div>
                <div className="text-xs text-muted mt-1">SKS Kuliah</div>
              </div>
              <div>
                <div className="text-2xl font-serif font-bold text-navy">{course.workload.labCredits}</div>
        ...
        <div className="bg-surface rounded-xl border border-border p-6">
          <h3 className="font-serif font-bold text-lg text-navy mb-5">Diagram Alur Prasyarat</h3>
        ...
        <div className="bg-surface rounded-xl border border-border p-5">
          ...
          >
            {addedCourseIds.has(course.id) ? (
              <><CheckCircle2 className="w-4 h-4" /> Sudah Ditambahkan ke Degree Planner</>
            ) : (
...
function PrerequisiteDiagram({ course }: { course: Course }) {
  if (!course.prerequisites || course.prerequisites.length === 0) {
    return (
      <div className="bg-background rounded-lg p-6 text-center">
        ...
        <p className="text-xs text-muted mt-1">Mata kuliah ini dapat langsung diambil.</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      ...
      >
        <defs>
          <marker id={`arrow-${course.id}`} markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
        ...
        {/* Edges: level0 → level1 */}
        {hasLevel0 && level1Nodes.map((p, pi) =>
          (p.prereqs || []).map(pp => {
        ...
        {hasLevel0 && level0Nodes.map((node, i) => {
          ...
          return (
            <g key={node.id}>
              ...
              <text x={x + NODE_W / 2} y={y + 46} textAnchor="middle"
        ...
        {(() => {
          ...
          return (
            <g>
              <rect x={x} y={y} width={NODE_W} height={NODE_H} rx="8"
                fill="#0f3e32" stroke="#d7b03d" strokeWidth="2" />
              <text x={x + NODE_W / 2} y={y + 17} textAnchor="middle"
